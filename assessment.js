...
//assessmentButton.addEventListener は、あるイベントが起きた際に、呼び出す関数を設定する記述するものです。
assessmentButton.addEventListener(
  ...
  () => {//無名関数 は名前を持たない関数の記述法です。
    ...
    '&ref_src=twsrc%5Etfw';
          //href属性は、リンク先のURLを指定する属性です
    anchor.setAttribute('href', hrefValue);//setAttributeは指定された要素の属性の値を設定します。
    anchor.setAttribute('class', 'twitter-hashtag-button');
    anchor.setAttribute('data-text', result);
    anchor.innerText = 'Tweet #あなたのいいところ';
/*VURI エンコードとは、URI のクエリに含めることのできない文字のために、 それらの文字を %（パーセント）という記号から始まる
 16 進数で表現して含めるようにするための変換方法のことです。 URL エンコード や パーセントエンコーディング などと言うこともあります。
JavaScript では、
...
function assessment(userName) {
  ...
  // 文字のコード番号の合計を回答の数で割ってあまりを求める。　そのあまりで出力するものを決める
